#!/bin/bash
# =============================================================================
# MSI Keyboard Controller - Сборка .deb пакета (с удалением старой версии)
# =============================================================================

set -e

echo "=============================================="
echo "  MSI Keyboard Controller - Сборка .deb пакета"
echo "=============================================="
echo ""
...
if dpkg -l | grep -q msi-keyboard-controller; then
    echo "Найдена старая версия. Удаляю..."
    sudo dpkg -r msi-keyboard-controller || true
    sudo apt-get autoremove -y || true
fi

# Очистка старых файлов
rm -rf msi-keyboard-controller
rm -f msi-keyboard-controller_*.deb

# Очистка старых правил udev
sudo rm -f /etc/udev/rules.d/99-msi-keyboard.rules
...
mkdir -p msi-keyboard-controller/DEBIAN
mkdir -p msi-keyboard-controller/usr/bin
mkdir -p msi-keyboard-controller/usr/share/applications
mkdir -p msi-keyboard-controller/usr/share/msi-keyboard-controller
mkdir -p msi-keyboard-controller/usr/share/icons/hicolor/256x256/apps
mkdir -p msi-keyboard-controller/usr/share/doc/msi-keyboard-controller
mkdir -p msi-keyboard-controller/etc/msi-keyboard-controller
...
cat > msi-keyboard-controller/DEBIAN/control << 'CONTROL_EOF'
Package: msi-keyboard-controller
Version: 1.0.1
Architecture: all
Maintainer: MSI Keyboard Controller Team <msi-keyboard@example.com>
Installed-Size: 250
Depends: python3, python3-tk, python3-usb, sudo
Section: utils
Priority: optional
Homepage: https://github.com/msi-keyboard-controller
Description: MSI Keyboard RGB Controller
 A GUI application for controlling RGB backlight on MSI keyboards.
 .
 Supports:
  * Static colors (RGB)
  * Animations (breathing, wave, rainbow)
  * Brightness control
  * Zone selection
  * Presets management
  * Save/Load to flash
 .
 Works with MSI MysticLight MS-1565 (Katana 17 B13V).
CONTROL_EOF
echo "✓ control создан"
echo ""

# =============================================================================
# 3. СОЗДАНИЕ POSTINST СКРИПТА
# =============================================================================
echo "📝 Создание DEBIAN/postinst..."
cat > msi-keyboard-controller/DEBIAN/postinst << 'POSTINST_EOF'
#!/bin/bash
set -e

echo "Настройка MSI Keyboard Controller..."

# Создание правила udev для доступа к USB
cat > /etc/udev/rules.d/99-msi-keyboard.rules << 'RULE_EOF'
# MSI MysticLight MS-1565
SUBSYSTEM=="usb", ATTR{idVendor}=="1462", ATTR{idProduct}=="1601", MODE="0666"
SUBSYSTEM=="usb", ATTR{idVendor}=="1462", ATTR{idProduct}=="1601", GROUP="plugdev"
RULE_EOF
...
mkdir -p /etc/msi-keyboard-controller

# Установка прав на исполняемый файл
chmod +x /usr/bin/msi-keyboard-controller

echo "✓ Настройка завершена"
exit 0
POSTINST_EOF
chmod 755 msi-keyboard-controller/DEBIAN/postinst
echo "✓ postinst создан"
echo ""

# =============================================================================
# 4. СОЗДАНИЕ PRERM СКРИПТА
# =============================================================================
echo "📝 Создание DEBIAN/prerm..."
cat > msi-keyboard-controller/DEBIAN/prerm << 'PRERM_EOF'
#!/bin/bash
set -e

echo "Удаление MSI Keyboard Controller..."

# Удаление правила udev
rm -f /etc/udev/rules.d/99-msi-keyboard.rules
udevadm control --reload-rules

echo "✓ Очистка завершена"
exit 0
PRERM_EOF
chmod 755 msi-keyboard-controller/DEBIAN/prerm
echo "✓ prerm создан"
echo ""

# =============================================================================
# 5. СОЗДАНИЕ ИСПОЛНЯЕМОГО СКРИПТА (с проверкой DISPLAY)
# =============================================================================
echo "📝 Создание лаунчера..."
cat > msi-keyboard-controller/usr/bin/msi-keyboard-controller << 'BIN_EOF'
#!/bin/bash
# =============================================================================
...
BIN_EOF
chmod 755 msi-keyboard-controller/usr/bin/msi-keyboard-controller
echo "✓ Лаунчер создан"
echo ""

# =============================================================================
# 6. СОЗДАНИЕ .DESKTOP ФАЙЛА
# =============================================================================
echo "📝 Создание .desktop файла..."
cat > msi-keyboard-controller/usr/share/applications/msi-keyboard-controller.desktop << 'DESKTOP_EOF'
[Desktop Entry]
Version=1.0
Type=Application
Name=MSI Keyboard Controller
Name[ru]=MSI Контроллер клавиатуры
Comment=Control RGB backlight on MSI keyboards
Comment[ru]=Управление RGB подсветкой клавиатуры MSI
Exec=msi-keyboard-controller
Icon=msi-keyboard-controller
Terminal=false
Categories=Utility;Settings;HardwareSettings;
Keywords=keyboard;rgb;msi;backlight;klaviatura;подсветка;
StartupNotify=true
DESKTOP_EOF
echo "✓ .desktop создан"
echo ""
...
cat > msi-keyboard-controller/usr/share/icons/hicolor/256x256/apps/msi-keyboard-controller.svg << 'ICON_EOF'
<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256" viewBox="0 0 256 256">
  <rect width="256" height="256" rx="30" fill="#1a1a1a"/>
  <rect x="30" y="50" width="196" height="156" rx="15" fill="#2d2d2d" stroke="#ff4444" stroke-width="4"/>
  <g fill="#ff4444">
    <rect x="45" y="65" width="30" height="30" rx="5"/>
    <rect x="85" y="65" width="30" height="30" rx="5"/>
    <rect x="125" y="65" width="30" height="30" rx="5"/>
    <rect x="165" y="65" width="30" height="30" rx="5"/>
    <rect x="45" y="105" width="30" height="30" rx="5"/>
    <rect x="85" y="105" width="30" height="30" rx="5"/>
    <rect x="125" y="105" width="30" height="30" rx="5"/>
    <rect x="165" y="105" width="30" height="30" rx="5"/>
    <rect x="45" y="145" width="30" height="30" rx="5"/>
    <rect x="85" y="145" width="30" height="30" rx="5"/>
    <rect x="125" y="145" width="30" height="30" rx="5"/>
    <rect x="165" y="145" width="30" height="30" rx="5"/>
  </g>
  <rect x="50" y="200" width="50" height="10" rx="3" fill="#ff0000"/>
  <rect x="103" y="200" width="50" height="10" rx="3" fill="#00ff00"/>
  <rect x="156" y="200" width="50" height="10" rx="3" fill="#0000ff"/>
  <text x="128" y="240" text-anchor="middle" fill="#ffffff" font-family="Arial" font-size="20" font-weight="bold">MSI</text>
</svg>
ICON_EOF
echo "✓ Иконка создана"
echo ""
...
# =============================================================================
# 8. СОЗДАНИЕ ОСНОВНОЙ ПРОГРАММЫ
# =============================================================================
echo "📝 Создание main.py..."
cat > msi-keyboard-controller/usr/share/msi-keyboard-controller/main.py << 'MAIN_EOF'
#!/usr/bin/env python3
"""
...
def check_and_request_permissions():
    if os.geteuid() != 0:
        ...
        try:
            env = os.environ.copy()
            # Сохраняем DISPLAY и XAUTHORITY
...
class PresetManager:
    ...
    def __init__(self, config_file='/etc/msi-keyboard-controller/presets.json'):
    ...
    def save_presets(self):
        try:
            ...
            home_config = os.path.expanduser('~/.msi-keyboard-controller/presets.json')
...
if __name__ == "__main__":
    main()
MAIN_EOF
chmod 644 msi-keyboard-controller/usr/share/msi-keyboard-controller/main.py
echo "✓ main.py создан"
echo ""
...
dpkg-deb --build msi-keyboard-controller

mv msi-keyboard-controller.deb msi-keyboard-controller_1.0.1_all.deb
...
echo "=============================================="
echo ""
echo "Файл: msi-keyboard-controller_1.0.1_all.deb"
echo ""
echo "Установка:"
echo "  sudo dpkg -i msi-keyboard-controller_1.0.1_all.deb"
echo ""
echo "Запуск из терминала:"
echo "  msi-keyboard-controller"
echo ""
echo "Или через меню приложений"
echo ""
echo "Удаление:"
echo "  sudo dpkg -r msi-keyboard-controller"
echo "=============================================="
